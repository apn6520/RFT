...
$(document).ready(function () {
    ...
    $("#btnArticleTitle").click(function () {
        $("#ArticleTitleModal").modal().on('shown.bs.modal', function () {
    ...
    $('#btnArticleTitleOk').click(function () {
        tinyMCE.triggerSave();
        var html = $("#ArticleTitle").val();
        $('#txtArticleTitle').val($(html).text());
    });


    if ($("#ArticleTitle").val().match(/<(\w+)((?:\s+\w+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)>/)) {
        tinyMCE.triggerSave();
        var html = $("#ArticleTitle").val();
        $('#txtArticleTitle').val($(html).text());
    } else {
        $('#txtArticleTitle').val($("#ArticleTitle").val());
    }

    $('#txtArticleTitle').prop('readonly', true);
    ...
    $('#btnAddotherAuthor').unbind("click").click(function () {
    ...
    if (($("#ddlRole option:selected").text() == "Associate") || ($("#ddlRole option:selected").text() == "Quality Analyst")) {
        IsQualityRole();
        IsViewEditable();
    }
    ...
    tinyMCE.init({
        ...
        mode: "textareas",
        theme: "modern",
        toolbar: "sizeselect | bold italic | fontselect | fontsizeselect | superscript | subscript | paste",
    ...
    tinyMCE.init({
        mode: "textareas",
        theme: "modern",
        toolbar: "sizeselect | bold italic | fontselect | fontsizeselect | superscript | subscript | charmap | paste",
        theme_advanced_toolbar_location: "top",
        theme_advanced_toolbar_align: "left",
        theme_advanced_statusbar_location: "bottom",
        theme_advanced_resizing: true,
        fontsize_formats: "8pt 10pt 12pt 14pt 18pt 24pt 36pt",
        editor_selector: "mceEditorForArticleTitle",
    ...
    $('#ddlJournalTitle').attr("disabled", "disabled");
    $('span').removeClass("dd-pointer dd-pointer-down");
    $('#ddlOverallAnalysis').css('pointer-events', 'none');
    var initialSubmissionDateOnly = $('#InitialSubmissionDate').val().replace(' 00:00:00', '');
    $('#InitialSubmissionDate').val(initialSubmissionDateOnly);
});//ready function end
...
function IsAddNewRevision() {
    ...
    if ($("#AddedNewRevision").is(":checked")) {
        ...
        $('#ddlOverallAnalysis').css('pointer-events', 'none');
        $("#btnArticleTitle").prop("disabled", false);
    }
    else {
        $('#ddlOverallAnalysis').css('pointer-events', 'default');
        ...
        if (role.toLocaleLowerCase() == "associate" && (isAssociateFinalSubmit.toLocaleLowerCase() == "true" || isQualityFinalSubmit.toLocaleLowerCase() == "true")) {
            ...
            $("#btnArticleTitle").prop("disabled", true);
        }
