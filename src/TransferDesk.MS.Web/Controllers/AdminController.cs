    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult JournalMaster(JournalVM journal)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (journal.ID != 0)
                {
                    _logger.Log(" Updating Journal details : " + userId);
                    if (_JournalRepository.IsJournalTitleStatusAvailable(journaltitlename, journal.IsActive, journal.ID,journal.AliasName))
                    {
                        _logger.Log(" Record for Journal Title  : "+ journaltitlename + " is already present" +  userId);
                    ...
                    else if (_JournalRepository.IsDuplicateAliasName(journal.AliasName, journal.ID,journal.JournalTitle))
                    {
                        _logger.Log(" Alias Name :  " + journal.AliasName + " is already present" + userId);
                        string message = "Alias Name : " + journal.AliasName + " is already present.";
                        TempData["msg"] = "<script>alert(\"" + message + "\");</script>";
                        return RedirectToAction("JournalMaster", "Admin", journal);
                    }
                    else
                    {
                        if (_JournalRepository.IsJournalStatusAvailable(journaltitlename, journal.IsActive,journal.AliasName))
                        {
                            _logger.Log(" Record for Journal Title  : " + journaltitlename + " is already present" + userId);
                            string message = "Record for Journal Title : " + journaltitlename + " is already present";
                            TempData["msg"] = "<script>alert(\"" + message + "\");</script>";
                        ...
                        else
                        {
                            ...
                            _JournalData.AliasName = journal.AliasName;
                            if (string.IsNullOrEmpty(journal.AliasName) || journal.AliasName.Trim() == "")
                                _JournalData.IsAliasActive = false;
                            else
                                _JournalData.IsAliasActive = true;
                            ...
                            _logger.Log(" Record for Journal Title  : " + journaltitlename + " is updated succesfully" + userId);
                ...
                else
                {
                    ...
                    {
                        _logger.Log(" Alias Name : " + journaltitlename + " is already present" + userId);
                        string message = "Alias Name : " + journal.AliasName + " is already present.";
                        TempData["msg"] = "<script>alert(\"" + message + "\");</script>";
                        return RedirectToAction("JournalMaster", "Admin", journal);
                    }
        ...
        public JsonResult IsAliasAvailable(string aliasName, int Id,string journalTitle )
        {
            return this.Json(_JournalRepository.IsDuplicateAliasName(aliasName, Id, journalTitle), JsonRequestBehavior.AllowGet);
        }
