...
namespace TransferDesk.MS.Web.Controllers
{
    public class ManuscriptLoginController : Controller
    {
        ...
        public ActionResult BookLogin(ManuscriptBookLoginVM manuscriptBookLoginVM)
        {
            ...
            manuscriptBookLoginVM.PageCount = manuscriptBookLoginVM.PageCount == null ? 0 : manuscriptBookLoginVM.PageCount;
        ...
        {
            return this.Json(_manuscriptDBRepositoryReadSide.GetAssociateName(searchText, "Associate"), JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult BookLogin(int? id, string jobtype)
        {
            ...
            if (id != null && id != 0 && jobtype == "book")
            {
                ...
                if (jobstatusisOnHold == false)
                {
                    ...
                    TempData["OnHoldMsg"] = "<script>alert(\"" + _errormsg + "\");</script>";
                    return RedirectToAction("BookLogin");
                }
        ...
        public bool CheckForAliasName(string msid, int journalTitleID)
                {

            var checkaliasstatus = ManuscriptLoginDbRepositoryReadSide.CheckAliasStatus(msid, journalTitleID);
            if (checkaliasstatus.ToString().ToLower().Trim() == "true")
            {  return true;}
            else
            {
            return false;
