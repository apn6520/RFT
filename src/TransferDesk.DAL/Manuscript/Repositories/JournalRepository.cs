...
namespace TransferDesk.DAL.Manuscript.Repositories
{
    public class JournalRepository
    {
        ...
        public bool IsJournalStatusAvailable(string journalname, bool status,string aliasName)
        {
            try
            {
                var result = from q in context.Journals
                             where (q.JournalTitle.ToLower().Trim().Equals(journalname.ToLower().Trim()) && q.IsActive.Equals(status) && q.AliasName.Trim().Equals(aliasName))
                             select q;
        ...
        public bool IsJournalTitleStatusAvailable(string title, bool active, int id, string aliasName)
        {
            if (id == 0)
            {
            ...
            else
            {
                var userRoles = from q in context.Journals
                                where q.JournalTitle.Trim().ToLower() == title.Trim().ToLower() && q.AliasName.Trim()== aliasName
                                select q;
        ...
        public bool IsAliasAvailable(string aliasName)
        {
            ...
            {
                var result = from q in context.Journals
                             where (q.AliasName.Equals(aliasName))
                             select q.AliasName;
        ...
        public bool IsDuplicateAliasName(string aliasName, int Id,string journalName)
        {
            if (Id != 0)
            {
                var previousAliasName = from q in context.Journals
                                        where (q.ID.Equals(Id)) 
                                        select q.AliasName;

                if (previousAliasName.ToList().Count > 0)
                {
                    if (previousAliasName.ToList()[0] == aliasName)
                        return false;
                    else
                    {
                        return IsAliasAvailable(aliasName);
                    }
                }
                else
                    return false;
