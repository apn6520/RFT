...
namespace TransferDesk.DAL.Manuscript.Repositories
{
    public class ManuscriptBookLoginRepository
    {
        ...
        public void UpdateManuscriptBookScreeningInfo(Entities.ManuscriptBookLogin manuscriptBookLogin)
        {
            try
            {
                if (manuscriptBookLogin.ID > 0)
                {
                    var result = context.ManuscriptBookLogin.Where(o => o.ID == manuscriptBookLogin.ID).FirstOrDefault();
                    result.PageCount = manuscriptBookLogin.PageCount;
                    result.ChapterTitle = manuscriptBookLogin.ChapterTitle;
                    context.SaveChanges();
                }
