...
namespace TransferDesk.DAL.Manuscript.Repositories
{
    public class ManuscriptLoginDBRepositoryReadSide : IDisposable
    {
        ...
        public bool IsBookCrestIDLogin(int serviceTypeId, int BookTitleId, string chapterNo, string chapterTitle, int ID)
        {
            ...
            {
                var check = (from s in manuscriptDataContextRead.ManuscriptBookLogin
                    where s.ID == ID && s.BookMasterID == BookTitleId && s.ChapterNumber == chapterNo
                    select s).ToList();
                if (check.Count() > 0)
                {

                    var currentServiceTypeId =
                        (from q in manuscriptDataContextRead.ManuscriptBookLogin where q.ID == ID select q.ServiceTypeID)
                            .FirstOrDefault();
                    ...
                    {
                        var resultrs = (from d in manuscriptDataContextRead.ManuscriptBookLogin
                                       where d.BookMasterID == BookTitleId && d.ChapterNumber == chapterNo &&
                                             d.ServiceTypeID == serviceTypeId && d.ManuscriptStatusID == 7 
                                       select d.CrestID).Count();
                        if (Convert.ToInt32(resultrs) == 0)
                            return false;
                        else
                            return true;
                    }
        ...
        public bool CheckAliasStatus(string msid, int journalTitleID)
        {

            var result = (from q in manuscriptDataContextRead.Journals
                where q.ID == journalTitleID
                select q.IsAliasActive).FirstOrDefault();
            if (result.ToString().ToLower().Trim() == "true")
            {
                var checkmsid = (from q in manuscriptDataContextRead.Journals
                    where q.ID == journalTitleID
                    select q.AliasName).FirstOrDefault();
                if (!String.IsNullOrEmpty(checkmsid))
                {
                    if (msid.ToLower().Trim().Contains(checkmsid.ToLower().Trim()))                 
                        return true;                  
                    else                  
                        return false;
                }
