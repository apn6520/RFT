...
namespace TransferDesk.DAL.Manuscript.Repositories
{
    public class ReviewerSuggestionDBRepositoryReadSide : IDisposable
    {
        ...
        public Entities.Employee GetAssociateInfoByName(string userName)
        {
            var _userName = userName != null ? new SqlParameter("userName", userName) : new SqlParameter("userName", typeof(global::System.String));
            List<Entities.Employee> empInfo =
                  this.ReviewerSuggestionDataContextRead.Database.SqlQuery<Entities.Employee>("exec pr_GetEmpInfoByName @userName", _userName).ToList();
            return empInfo.FirstOrDefault();
        }
        ...
        {
            var date = (from q in ReviewerSuggestionDataContextRead.MSReviewersSuggestion
                        where q.MSID == MSID
                        select q.QualityStartDate).FirstOrDefault();
            if (date != null)
            {
