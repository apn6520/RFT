...
namespace TransferDesk.DAL.Manuscript.Repositories
{
    class MSReviewerInfo
    {
        ...
        internal void RemoveReviewerTile(int reviewerId, string articleTitle, string user)
        {
            ...
            if (titleInfo != null)
            {
                var titleReviewerLink = context.TitleReviewerlink.Where(
                    x => x.ReviewerMasterID == reviewerId && x.TitleMasterID == titleInfo.TitleID).ToList();

                foreach (var item in titleReviewerLink)
                {
                    item.IsActive = false;
                    item.ModifiedBy = user.Trim();
